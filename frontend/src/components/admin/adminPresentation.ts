...
export function auditActionLabel(action?: string) {
  if (action === 'admin.agent.status.updated') return '修士状态更新'
  if (action === 'admin.agent.growth.evaluated') return '成长评估'
  if (action === 'admin.agent.growth.skill_draft.updated') return '法卷草稿审核'
...
export function auditResourceLabel(resourceType?: string | null) {
  if (resourceType === 'agent') return '修士'
  if (resourceType === 'agent_growth') return '成长档案'
  if (resourceType === 'agent_growth_skill_draft') return '法卷草稿'
