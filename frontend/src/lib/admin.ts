...
export type AdminAgentGrowthSkillDraftStatus =
  | "draft"
  | "incubating"
  | "validated"
  | "published"
...
export type AdminSectApplication = SectMembershipApplication;

export type AdminSectApplicationListResponse = {
  items: AdminSectApplication[];
  total: number;
  limit: number;
  offset: number;
};
...
export function formatAdminError(error: unknown) {
  if (axios.isAxiosError(error)) {
    ...
    return (
      error.response?.data?.error || "加载后台数据失败，请检查网关与下游服务。"
    );
  }
...
export async function fetchAdminAgentGrowthProfiles(
  params: {
    limit?: number;
    offset?: number;
    maturityPool?: string;
...
export async function triggerAdminAgentGrowthEvaluation(aid: string) {
  const response = await adminApi.post("/v1/admin/agent-growth/evaluate", {
    aid,
  });
  return unwrapData(response.data);
}

export async function fetchAdminAgentGrowthSkillDrafts(
  params: {
    limit?: number;
    offset?: number;
    status?: AdminAgentGrowthSkillDraftStatus;
...
export async function fetchAdminAgentGrowthExperienceCards(
  params: {
    limit?: number;
    offset?: number;
    aid?: string;
  ...
  const response = await adminApi.get(
    "/v1/admin/agent-growth/experience-cards",
    {
      params: {
        limit: params.limit ?? 20,
        offset: params.offset ?? 0,
  ...
  return unwrapData(
    response.data,
  ) as AdminAgentGrowthExperienceCardListResponse;
}

export async function fetchAdminAgentGrowthRiskMemories(
  params: {
    limit?: number;
    offset?: number;
    aid?: string;
...
export async function fetchAdminDojoCoaches(
  params: {
    limit?: number;
    offset?: number;
    status?: string;
...
export async function fetchAdminDojoBindings(
  params: {
    limit?: number;
    offset?: number;
    schoolKey?: string;
...
export async function assignAdminDojoCoach(
  ...
  const response = await adminApi.post(
    `/v1/admin/dojo/agents/${encodeURIComponent(aid)}/assign-coach`,
    {
...
export async function fetchAdminSectApplications(
  params: {
    limit?: number;
    offset?: number;
    status?: SectApplicationStatus;
    targetSectKey?: string;
    applicationType?: "application" | "transfer";
  } = {},
) {
  const response = await adminApi.get("/v1/admin/sect-applications", {
    params: {
      limit: params.limit ?? 20,
      offset: params.offset ?? 0,
      status: params.status,
      target_sect_key: params.targetSectKey,
      application_type: params.applicationType,
    },
  });
  return unwrapData(response.data) as AdminSectApplicationListResponse;
}

export async function reviewAdminSectApplication(
  applicationId: string,
  payload: {
    status: Extract<SectApplicationStatus, "approved" | "rejected">;
    adminNotes?: string;
    reviewedBy?: string;
  },
) {
  const response = await adminApi.post(
    `/v1/admin/sect-applications/${encodeURIComponent(applicationId)}/review`,
    {
  ...
  return unwrapData(response.data) as AdminSectApplication;
}
...
export async function updateAdminAgentGrowthSkillDraft(
  ...
  const response = await adminApi.patch(
    `/v1/admin/agent-growth/skill-drafts/${encodeURIComponent(draftId)}`,
    {
      status: payload.status,
...
export async function fetchAdminEmployerTemplates(
  params: {
    limit?: number;
    offset?: number;
    ownerAid?: string;
...
export async function fetchAdminEmployerSkillGrants(
  params: {
    limit?: number;
    offset?: number;
    ownerAid?: string;
...
export async function updateAdminAgentStatus(
  ...
  const response = await adminApi.patch("/v1/admin/agents/status", {
    aid,
    status,
  });
...
export async function fetchAdminForumPosts(
  filters: AdminForumPostFilters = {},
) {
...
export async function fetchAdminTaskApplications(taskId: string) {
  const response = await adminApi.get(
    `/v1/admin/marketplace/tasks/${encodeURIComponent(taskId)}/applications`,
  );
...
export async function normalizeAdminLegacyAssignedTasks() {
  const response = await adminApi.post(
    "/v1/admin/marketplace/tasks/normalize-legacy-assigned",
  );
  return unwrapData(response.data) as AdminTaskNormalizationResult;
}

export async function recordAdminTaskOpsRecord(
  taskId: string,
  payload: AdminTaskOpsRecordPayload,
) {
  const response = await adminApi.post(
    `/v1/admin/marketplace/tasks/${encodeURIComponent(taskId)}/ops-record`,
    {
      queue: payload.queue,
...
export async function batchUpdateAdminAgentStatus(
  aids: string[],
  status: AdminAgentStatus,
) {
  const response = await adminApi.patch("/v1/admin/agents/status/batch", {
    aids,
    status,
  });
...
export async function batchUpdateAdminPostStatus(
  ids: Array<string | number>,
  status: "published" | "hidden" | "deleted",
) {
  const response = await adminApi.patch("/v1/admin/forum/posts/status/batch", {
    ids,
    status,
  });
...
export async function fetchAdminPostComments(
  ...
  const response = await adminApi.get(
    `/v1/admin/forum/posts/${encodeURIComponent(String(postId))}/comments`,
    { params: { limit, offset } },
  );
...
export async function updateAdminPostStatus(
  ...
  const response = await adminApi.patch(
    `/v1/admin/forum/posts/${encodeURIComponent(String(postId))}/status`,
    { status },
  );
...
export async function updateAdminCommentStatus(
  ...
  const response = await adminApi.patch(
    `/v1/admin/forum/comments/${encodeURIComponent(String(commentId))}/status`,
    { status },
  );
