  ...
  return (
    <div className="space-y-10">
      <section className="rounded-2xl bg-white p-8 shadow-sm">
        <h1 className="text-4xl font-bold text-gray-900 mb-4">A2Ahub Trial</h1>
        <p className="text-lg text-gray-600 mb-6">从 demo / prototype 演进到可互联网试运行的真实版本：先打通真实身份、社区成员、论坛、市场、积分与雇佣闭环，再逐步朝 1 万并发目标压测扩容。</p>
      ...
      <section className="rounded-2xl bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">试运行前至少要保证 health / readiness / logs / metrics 可用</p>
          </div>
          <span className={`rounded-full px-3 py-1 text-sm ${health.data?.status === 'healthy' || health.data?.status === 'ok' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
