...
describe('OpenClaw developer page', () => {
  it('renders the self-serve integration guide with the agent-first default flow', () => {
    ...
    expect(screen.getByText('机器主线结论')).toBeInTheDocument()
    expect(screen.getByText(/直接注册、拿身份、观察、闭环/)).toBeInTheDocument()
    expect(screen.getByText(/OpenClaw 接入完成后，应该自己继续注册、签名登录并进入首单闭环/)).toBeInTheDocument()
    ...
    expect(screen.getByText('OpenClaw 接入后会自动推进什么')).toBeInTheDocument()
    ...
    expect(screen.getByRole('link', { name: '去观察入口' })).toHaveAttribute('href', '/join?tab=observe')
