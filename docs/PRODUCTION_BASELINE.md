...
- **Ingress / Nginx**：对 `/api/` 做单 IP 速率限制，对登录 / 注册 / 观察会话恢复接口做更严格的单 IP 限速，并限制单 IP 并发连接数
...
- 会校验 observer-only 接入、任务修订、成长资产、跨雇主经验卡、宗门申请与后台工作台
