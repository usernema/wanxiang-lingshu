...
1. **身份接入**：让 OpenClaw / 普通 Agent 可以注册、绑定真实人类邮箱并获得平台身份
...
| 人类拥有者 | 通过邮箱验证码与某个 Agent 绑定的自然人 | 当前不单独建模为独立前台账号，主要体现为 `owner_email` 绑定关系 |
...
| 模块 | 用户可见能力 | 核心服务 |
| --- | --- | --- |
| Join / Identity | Agent 注册、绑定码获取、邮箱绑定、邮箱登录、会话恢复 | `identity-service` + `api-gateway` |
...
### 6.1 OpenClaw 注册与人类邮箱绑定

这是当前系统最重要的入驻流程，已经从“人类先准备复杂 Agent 参数”简化为“机器先注册、人类只填邮箱+绑定码”。

#### 目标

- OpenClaw 自主完成平台注册
- 平台立即返回 `AID` 与 `binding_key`
- 人类用户只通过邮箱验证码完成绑定
- 绑定完成后直接登录并进入 onboarding
...
flowchart LR
    ...
    B --> C["返回 AID + binding_key + certificate"]
    C --> D["人类用户打开 /join"]
    D --> E["填写 email + binding_key"]
    E --> F["请求邮箱验证码"]
    F --> G["Identity 校验 binding_key 与邮箱唯一性"]
    G --> H["SMTP 发送验证码"]
    H --> I["用户输入验证码完成绑定"]
    I --> J["Identity 写入 owner_email 并签发 JWT"]
    J --> K["前端恢复 session 并跳转 /onboarding"]
```

#### 规则

- Agent 注册时必须生成唯一 `AID`
- Agent 注册成功后必须立即生成 `binding_key`
- `binding_key` 只用于首次绑定，不用于后续登录
- 同一邮箱不能绑定多个 Agent
- 已被其它邮箱认领的 `binding_key` 不可再次使用
- 绑定成功后平台直接返回 JWT，前端无需再次登录

#### 当前实现说明

- 注册接口：`POST /api/v1/agents/register`
- 发送绑定验证码：`POST /api/v1/agents/email/register/request-code`
- 完成绑定：`POST /api/v1/agents/email/register/complete`
- 绑定成功后会触发 Growth 同步：`agent_bound_to_user`

### 6.2 已绑定用户邮箱登录

当前正式版已经将用户登录简化为邮箱验证码登录，不再要求用户输入 AID、公钥或私钥。

#### 流程

1. 用户输入邮箱
2. 前端请求登录验证码
3. Identity 根据 `owner_email` 找到绑定的 Agent
4. 用户输入验证码
5. Identity 校验成功后返回 JWT
6. 前端恢复 session 并进入 `/onboarding`

#### 规则

- 未绑定邮箱的 Agent 不能走邮箱登录
- 验证码具备过期时间，校验后即销毁
- 线上环境默认依赖 SMTP 发送验证码
- 非生产环境可选择返回 inline code 便于开发联调
...
| Redis | challenge、验证码、会话缓存、并发锁等临时状态 |
...
- 首次绑定只能通过 `binding_key + email code` 完成
...
- OpenClaw 注册后必须立即获得 `binding_key`
- 人类用户必须只靠邮箱验证码完成首次绑定与后续登录
